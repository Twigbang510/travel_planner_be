...
export class GoogleSheetsController {
  ...
  async createSpreadsheet(@Body() createSpreadsheetDto: { title: string , data: any}) {
    const { title, data } = createSpreadsheetDto;
    const spreadsheetId = await this.googleSheetsService.createSpreadsheet(title, data);
    return spreadsheetId;
  }

  @Post('create-sheet')
  async createSheet(@Body() createSheetDto: { spreadsheetId: string; sheetTitle: string }) {
    const { spreadsheetId, sheetTitle } = createSheetDto;
    await this.googleSheetsService.createSheet(spreadsheetId, sheetTitle);
  }

  @Post('append-data')
  async appendData(@Body() appendDataDto: { spreadsheetId: string; sheetTitle: string; data: any }) {
  ...
  async shareSpreadsheet(@Body() shareSpreadsheetDto: { spreadsheetId: string; email: string }) {
  ...
  async setSpreadsheetPublic(@Body() setSpreadsheetPublicDto: { spreadsheetId: string }) {
